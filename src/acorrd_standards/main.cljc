(ns acorrd-standards.main
  "Kotodama WASM entrypoint for the Acorrd_standards clean-room actor (L4) — Clojure port.
  ...
  facts (`acorrd_standards.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [kotoba.lang.text :as str]))

(def ns-prefix "acorrd_standards")
(def tier "L4")
...
(def entity-specs
  [{:entity "Account"     :plural "accounts"     :id-prefix "acorrdst_acc"
    :fields [:holderId :type :balance :currency :status] :required [:type :balance]
    :coerce {:balance :float}           :refs {}}
   {:entity "Loan"        :plural "loans"        :id-prefix "acorrdst_loa"
...
(defn as-int-kernel
  ...
  of acorrd-standards.main/as-int stays here as the retained oracle."
  [v]
  (if (pos? v) v 0))
...
(defn emit-facts
  "EAVT facts for one record: {\"acorrd_standards.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "acorrd_standards-compat" :tier tier :entities entities} 200])
...
(defn start! [] :acorrd_standards-compat/ready)
